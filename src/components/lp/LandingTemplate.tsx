...
// Carrossel com peek: mostra parte do proximo item para indicar que tem mais
const Carousel = ({ children, peekWidth = "82vw", desktopCols = 1, maxWidthDesktop = "1100px" }: {
  ...
  return (
    <div>
      ...
      <div className="md:hidden" ref={ref} style={{
        display:"flex", overflowX:"scroll", scrollSnapType:"x mandatory",
        WebkitOverflowScrolling:"touch", scrollbarWidth:"none", gap:"10px", paddingRight:"24px",
      }}>
        {children.map((child,i) => (
          <div key={i} style={{ flexShrink:0, width:peekWidth, scrollSnapAlign:"start" }}>{child}</div>
        ))}
      </div>
      {/* Dots indicadores no mobile */}
      {children.length > 1 && (
        <div className="md:hidden" style={{ display:"flex", justifyContent:"center", gap:"5px", marginTop:"12px" }}>
          {children.map((_,i) => (
            <div key={i} style={{
              width: active===i ? "16px" : "5px", height:"5px",
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor:"#F5F1EB" }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between" style={{ backgroundColor:"#2C2C2A", padding:"14px 24px" }}>
        ...
        <a href="tel:+15089829675" style={{ color:"#C4291C", fontWeight:600, fontSize:"13px" }}>508 982 9675</a>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-[55fr_45fr]" style={{ paddingTop:"56px" }}>
        ...
        <div className="hidden md:flex px-12 py-[60px] flex-col justify-center" style={{ backgroundColor:"#F5F1EB" }}>
          <div className="w-full max-w-md mx-auto"><LPForm service={service}/></div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor:"#F5F1EB", padding:"clamp(36px,5vw,64px) 24px" }}>
        <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
          <h2 className="font-display" style={{ fontWeight:900, fontSize:"clamp(26px,3vw,36px)", color:"#1A1A1A", letterSpacing:"-0.02em", marginBottom:"20px" }}>What's included.</h2>
          <Carousel peekWidth="72vw" desktopCols={4} maxWidthDesktop="1100px">
            {included.map(item => (
              <div key={item.title} style={{ backgroundColor:"#FFFFFF", border:"1px solid #E8E2D8", borderRadius:"8px", padding:"16px", display:"flex", alignItems:"flex-start", gap:"10px", height:"100%" }}>
                <Check size={16} style={{ color:"#C4291C", flexShrink:0, marginTop:"2px" }}/>
                <p style={{ color:"#1A1A1A", fontSize:"13px", fontWeight:600, lineHeight:1.4 }}>{item.title}</p>
              </div>
            ))}
          </Carousel>
        </div>
      ...
      <section style={{ backgroundColor:"#E8E2D8", padding:"clamp(36px,5vw,64px) 24px" }}>
        <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
          <h2 className="font-display" style={{ fontWeight:900, fontSize:"clamp(26px,3vw,36px)", color:"#1A1A1A", letterSpacing:"-0.02em", marginBottom:"20px" }}>How it works.</h2>
      ...
      <section style={{ backgroundColor:"#FFFFFF", padding:"clamp(36px,5vw,64px) 24px" }}>
        <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
          <h2 className="font-display" style={{ fontWeight:900, fontSize:"clamp(26px,3vw,36px)", color:"#1A1A1A", letterSpacing:"-0.02em", marginBottom:"20px" }}>What our clients say.</h2>
          <Carousel peekWidth="80vw" desktopCols={3} maxWidthDesktop="1100px">
            {reviews.map(r => (
              <div key={r.name} style={{ backgroundColor:"#F5F1EB", border:"1px solid #E8E2D8", borderRadius:"8px", padding:"20px" }}>
                ...
                <p className="mt-3" style={{ color:"#6B6560", fontSize:"12px", fontWeight:600 }}>{r.name}</p>
              </div>
            ))}
          </Carousel>
        </div>
      ...
      <section style={{ backgroundColor:"#F5F1EB", padding:"clamp(36px,5vw,64px) 24px" }}>
        <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
          <h2 className="font-display" style={{ fontWeight:900, fontSize:"clamp(26px,3vw,36px)", color:"#1A1A1A", letterSpacing:"-0.02em", marginBottom:"20px" }}>Recent work.</h2>
